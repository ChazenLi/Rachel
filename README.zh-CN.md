...
论文 Figure 2 展示了目标 `n1_366` 经七步转化连接至六个终端前体的完整路线。
路线旁的三个决策摘要，具体说明局部反应选择如何服务于整体合成策略。

![论文 Figure 2：七步完整逆合成路线及三个关键化学决策摘要。](docs/assets/rachel-route-decisions.png)

[查看原尺寸图片](docs/assets/rachel-route-decisions.png) ·
[打开 PDF](docs/assets/rachel-route-decisions.pdf)

左侧从目标向下追溯至前体，反应名称表示相应的正向转化。右侧解释三个选择：

- **安排酚羟基的保护与释放：**在上游转化中保留甲醚，随后在合成过程中释放酚羟基。
- **引入 C3 羧基：**通过先甲酰化、后氧化，为后续酰胺偶联提供反应位点。
- **构建吲哚核心：**通过 Fischer 成环建立骨架，再进行后续官能化。

每个摘要都对应记录中的选择理由、对路线的影响和考虑过的替代方案。
图中展示的是规划方案；更多记录路线可在[完整 Atlas](#route-atlas)中逐一查看。
